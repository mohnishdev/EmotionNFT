// SPDX-License-Identifier: MIT

pragma solidity ^0.8.18;

import {Test , console} from "forge-std/Test.sol";
import {MoodNft} from "../src/MoodNft.sol";

contract MoodNftTest is Test {
    MoodNft moodNft;

    address USER = makeAddr("user");
    
    receive() external payable {}

    string public constant HAPPY_MOOD_URI =
        "data:application/json;base64,";

    string public constant SAD_MOOD_URI =
        "data:application/json;base64,";

    function setUp() public {
        moodNft = new MoodNft(HAPPY_MOOD_URI, SAD_MOOD_URI);
    } 

    function testViewTokenUri() public {
        vm.deal(USER, 1 ether);
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        console.log(moodNft.tokenURI(0));
    }

    function testMintPriceIsCorrect() public view {
        assert(moodNft.getMintPrice() == 0.01 ether);
    }

    function testCanMintWithCorrectPayment() public {
        vm.deal(USER, 1 ether);
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        assert(moodNft.balanceOf(USER) == 1);
        assert(moodNft.ownerOf(0) == USER);
    }

    function testRevertsWithInsufficientPayment() public {
        vm.deal(USER, 1 ether);
        vm.prank(USER);
        vm.expectRevert(MoodNft.MoodNft__InsufficientPayment.selector);
        moodNft.mintNft{value: 0.005 ether}();
    }

    function testCanFlipMood() public {
        vm.deal(USER, 1 ether);
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        
        // Initially should be happy (default state)
        string memory initialUri = moodNft.tokenURI(0);
        
        // Flip mood
        vm.prank(USER);
        moodNft.flipMood(0);
        
        // URI should be different now
        string memory newUri = moodNft.tokenURI(0);
        assert(keccak256(abi.encodePacked(initialUri)) != keccak256(abi.encodePacked(newUri)));
    }

    function testRevertsWhenNonOwnerTriesToFlipMood() public {
        vm.deal(USER, 1 ether);
        address nonOwner = makeAddr("nonOwner");
        
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        
        vm.prank(nonOwner);
        vm.expectRevert(MoodNft.MoodNft__CantFlipMoodIfNotOwner.selector);
        moodNft.flipMood(0);
    }

    function testOwnerCanSetMintPrice() public {
        uint256 newPrice = 0.02 ether;
        moodNft.setMintPrice(newPrice);
        assert(moodNft.getMintPrice() == newPrice);
    }

    function testNonOwnerCannotSetMintPrice() public {
        vm.prank(USER);
        vm.expectRevert();
        moodNft.setMintPrice(0.02 ether);
    }

    function testOwnerCanWithdraw() public {
        vm.deal(USER, 1 ether);
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        
        address owner = moodNft.owner();
        vm.deal(owner, 1 ether); // Give owner some ETH to receive the withdrawal
        uint256 ownerBalanceBefore = owner.balance;
        moodNft.withdraw();
        uint256 ownerBalanceAfter = owner.balance;
        
        assert(ownerBalanceAfter > ownerBalanceBefore);
        assert(ownerBalanceAfter == ownerBalanceBefore + 0.01 ether);
    }

    function testNonOwnerCannotWithdraw() public {
        vm.deal(USER, 1 ether);
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        
        vm.prank(USER);
        vm.expectRevert();
        moodNft.withdraw();
    }

    function testTokenCounterIncrements() public {
        assert(moodNft.getTokenCounter() == 0);
        
        vm.deal(USER, 1 ether);
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        
        assert(moodNft.getTokenCounter() == 1);
        
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
        
        assert(moodNft.getTokenCounter() == 2);
    }

    function testEmitsCreatedNFTEvent() public {
        vm.deal(USER, 1 ether);
        vm.expectEmit(true, false, false, true);
        emit MoodNft.CreatedNFT(0);
        
        vm.prank(USER);
        moodNft.mintNft{value: 0.01 ether}();
    }
}